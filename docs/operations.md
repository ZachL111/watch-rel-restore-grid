...
`watch-rel-restore-grid` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for restore scenarios with log and snapshot fixtures, replay consistency checks, and local-only command execution.
...
The score is deterministic. The current threshold is 161. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 3.
